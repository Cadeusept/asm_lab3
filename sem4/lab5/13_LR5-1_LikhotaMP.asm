...
cycle:
 ...
 ;       add     rcx, 8                                  ; Arg 1 - IN :
        ...
        mov     rbx,[BootServices]
        sub     rsp, 4*8                                ; "shadow space" for Rcx, Rdx, R8, R9
        call    qword[rbx + EFI_BOOT_SERVICES_TABLE.HandleProtocol]
    ...
    ;     Status = SimpleTextInEx->RegisterKeyNotify(
        ...
        mov rcx, [UnicodeCount]
        mov rax, Unicodes
        register_key_notify_loop:
            mov bx, word[rax]
            inc rax
            inc rax
            push rax
            mov word[Key.UnicodeChar], bx
            push rcx

            mov rcx, [SimpleTextInEx]
            lea rdx, [KeyData]
            lea r8, [test_func]
            lea r9, [Handle]
...
SystemTable     dq              ?
BootServices    dq              ?
; ScanCodes dw 1Eh,30h,2Eh,20h,12h,21h,22h,23h,17h,24h,25h,26h,32h,31h,18h,19h,10h,13h,1Fh,14h,16h,2Fh,11h,2Dh,15h,2Ch,0Bh,02h,03h,04h,05h,06h,07h,08h,09h,0Ah
Unicodes dw 30h, 31h, 32h, 33h, 34h, 35h, 36h, 37h, 38h, 39h
UnicodeCount dq 0x10
; ScanCodeCount dq 24h
Text            du              'Hello DXE',13,10,0    ; 13 and 10 are Line Feed and Form Feed characters
_OK             du              'OK',13,10,0
_OKOK             du              'OKOK',13,10,0
